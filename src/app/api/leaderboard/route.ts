  ...
  try {
    ...
    for (const file of userFiles) {
      try {
        ...
        // Get commits for this specific file (get more to find the right one)
        const commitsUrl = `https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/commits?path=${encodeURIComponent(file.path)}&per_page=10`;
        ...
        // Find the latest commit that matches the [Difficulty] Problem Name format
        const validCommit = commits.find(commit => {
          const message = commit.commit.message;
          return /\[(Easy|Medium|Hard)\]/i.test(message);
        });
        
        if (!validCommit) {
          debugInfo.push({
            path: file.path,
            reason: 'No commits with [Difficulty] format found',
            type: 'skipped'
          });
          continue;
        }
        
        // Use the valid commit instead of just the latest
        const latestCommit = validCommit;
